			...
			success : function(response) {
				...
				if(!(respText.currentMonth==0&&respText.lastMonth==0))
				{
					if(respText.currentMonth>respText.lastMonth){
						var gain=((respText.currentMonth-respText.lastMonth)*100/respText.lastMonth).toFixed(2)+"%";
...
/*				if (respText.length == null || respText.length == "") {
	...
	if( $('#project-flow').length > 0 ){
		...
		var selectValue=$('.selectpicker')[0][$('.selectpicker')[0].selectedIndex].innerHTML;
		myChart.clear();
		Ext.Ajax.request({
			async : false,
